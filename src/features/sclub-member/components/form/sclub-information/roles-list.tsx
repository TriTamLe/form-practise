...
export const RoleLists = () => {
  ...
  const hasRole = useWatch(SCLUB_MEMBER_KEYS.HAS_ROLE, form)
  const roles = useWatch(SCLUB_MEMBER_KEYS.ROLES, form)
  ...
  return (
    <>
      <CustomFormItem name={SCLUB_MEMBER_KEYS.HAS_ROLE} label={LABELS.HAS_ROLE}>
        <Switch checkedChildren={'YES'} unCheckedChildren={'NO'} />
      </CustomFormItem>

      {hasRole && (
        <Form.List
          name={SCLUB_MEMBER_KEYS.ROLES}
              ...
              <div className='w-full flex flex-col gap-5'>
                {fields.map(({ key, name }) => (
                  <Space key={key} align='baseline' className='w-full flex'>
                    <CustomFormItem
                      name={[name, SCLUB_MEMBER_KEYS.TERM]}
                      label={LABELS.TERM}
                    ...
                    <CustomFormItem
                      name={[name, SCLUB_MEMBER_KEYS.ROLE]}
                      label={LABELS.ROLE}
                      rules={rules.role as Rule[]}
                      required>
                      <Select
                        options={SCLUB_POSITIONS_OPTIONS}
